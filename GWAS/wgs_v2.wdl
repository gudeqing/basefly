...
workflow wgs_wf {
    ...
    scatter (interval_idx in range(interval_number)) {
        call deal_interval_idx {
            input:
        ...
        call combine_calls {
            input:
                ...
                interval_idx = deal_interval_idx.intervalidx,
        ...
        scatter (vartype in variant_types) {
            ...
            call hard_filter_calls {
                input:
                    ...
                    vartype = vartype,
                    filter_str = if vartype == "SNP" then snv_hard_filter else indel_hard_filter,
                    contig = contig[0],
                    outdir = outdir,
                    genome_fa = ref_fa,
                    ref_fai = ref_fai,
    ...
    call merge_calls {
        input:
            queue = queue,
            input_vcfs = flatten(sort_vcf.output_vcf),
            outdir = outdir
    }
...
task deal_interval_idx{
    ...
    command <<<
       python <<CODE
       print(str(~{interval_idx}).zfill(4))
       CODE
    >>>

    output{
        String intervalidx = read_string(stdout())
    }
...
task combine_calls{
    ...
    command <<<
      singularity exec -B /mnt,/data ~{singularity} gatk --java-options '~{java_opts}' CombineGVCFs \
            ...
            ~{sep=" " prefix('-V ',gvcfs)} \
            -L ~{interval_dir}/~{interval_idx}-scattered.interval_list
    >>>
