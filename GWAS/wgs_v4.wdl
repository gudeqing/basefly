...
#        Array[String] plugin_names = ['Frameshift', 'Wildtype']
    ...
    parameter_meta {
        ...
        cpus: {prefix: "--fork ", type: "int", level: "required", default: "4", range: "None", array: "False", desc: "Use forking(multi-cpu/threads) to improve script runtime"}
...
task prepare_gwas_input {
    input {
        String study_name = "gwas_study"
        File phenotype = "/mnt/nas_101/juyyong/GWAS/original/result_0418/phone_1200.txt"
        File keep_han_site = "/mnt/nas_002/gwas/dailsun_1200/new/keep.han.txt"
        File gvcf
        String queue
        String outdir
    }
    ...
    command <<<
        mkdir -p ~{outdir}/gwas_inputs
        echo -e "~{study_name}\t~{gvcf}\t~{phenotype}\t~{keep_han_site}" > ~{outdir}/gwas_inputs/gwas.input.txt
    >>>

    output {
        File gwas_input_file = "~{outdir}/gwas_inputs/gwas.input.txt"
    }
}