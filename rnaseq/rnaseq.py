...
def pipeline():
    ...
    * 分析结果：主要分析结果将汇总于输出目录如Result/Report
    ...
    ref_dir: 分析流程需要的参考文件如参考基因组索引等所在目录，该目录可以通过如下方式准备：
        ...
        4. 制作核糖体rRNA的interval文件，放置ref_dir目录下，通过ref_flat参数指定文件即可
        5. 下载arcasHLA的参考目录（参考https://github.com/RabadanLab/arcasHLA），放置ref_dir目录下，通过hla_db参数指定文件即可
    hla_db: 赋值为空字符串时，则跳过HLA定型分析步骤。默认参考文件在ref_dir下的“HLA_reference/arcasHLA_dat”
    dbsnp,known_indels,known_mills 等文件仅当有sentieon可用时才需要，仅用于SNP/Indel分析用途, 一般不使用RNASeq数据检测SNP/Indel
    """
    wf.init_argparser()
    # add workflow args
    ...
    wf.add_argument('-ref_dir', default='/enigma/datasets/*/GRCh37_gencode_v19_CTAT_lib_Mar012021.plug-n-play/', help='The directory containing all the needed reference files or directories.')
    ...
    wf.add_argument('-gtf', default='ctat_genome_lib_build_dir/ref_annot.gtf', help='genome annotation file path relative to ref_dir')
    wf.add_argument('-ref_flat', default='picard_inputs/hg19.refFlat.txt.gz', help='gene model file, genome annotation file, needed for QC. Download from: http://hgdownload.cse.ucsc.edu/goldenPath/hg19/database/refFlat.txt.gz')
