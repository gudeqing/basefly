    ...
    for sample_names, bams in parse_bam_list(wf.args.bams).items():
        ...
        if wf.args.genome_pep:
            ...
            if ms_data and (tumor_sample in ms_data):
                ...
                ms_search_task, args = wf.add_task(comet(), tag=tumor_sample+'MHC1', depends=[convert2mgf_task, find_novel_peptide_task])
                args['param_file'].value = wf.args.comet_params
            ...
            if valid_alleles:
                ...
                args['input'].value = find_novel_peptide_task.outputs['mhc1_faa']
                args['output'].value = tumor_sample + '.MixMHC2pred.txt'
                args['alleles'].value = valid_alleles
            else:
                print(f'skip MixMHC2Pred task for {tumor_sample} since no valid HLA-gene combination available')
