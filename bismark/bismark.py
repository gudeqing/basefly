...
def bismark_alignment():
    ...
    cmd.outputs['bam'] = Output(value='*.bam', report=False)
...
def bismark_methylation_extractor():
    ...
    cmd.outputs['CHG_OB'] = Output(value='CHG_OB_*.txt.gz', report=True)
    cmd.outputs['CHG_OT'] = Output(value='CHG_OT_*.txt.gz', report=True)
    cmd.outputs['CHH_OB'] = Output(value='CHH_OB_*.txt.gz', report=True)
    cmd.outputs['CHH_OT'] = Output(value='CHH_OT_*.txt.gz', report=True)
    cmd.outputs['CpG_OB'] = Output(value='CpG_OB_*.txt.gz', report=True)
    cmd.outputs['CpG_OT'] = Output(value='CpG_OT_*.txt.gz', report=True)
    return cmd
...
def bismark2summary():
    ...
    cmd.outputs['summary_txt'] = Output(value='{basename}.txt', report=True)
