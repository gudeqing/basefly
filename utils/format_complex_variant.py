    ...
    with open(infile) as fr:
        while True:
            ...
            if line.startswith("[Small Variants]"):
                break
        table = pd.read_csv(fr, header=0, skip_blank_lines=True, sep='\t')
...
def small_variant_tsv_to_vcf(infile, out=None, sample_name=None):
    ...
    for idx, row in table.iterrows():
        ...
        record.pos = row['Genomic Position']
        ...
        record.info.update(dict(
            Gene=str(row['Gene']),
            DP=row['Depth'],
            AF=row['Allele Frequency'],
            pHgvs=str(row['P-Dot Notation'])
        ))
        record.samples[sample_name]['DP'] = row['Depth']
        record.samples[sample_name]['AF'] = row['Allele Frequency']
        vcf.write(record)
    else:
        vcf.close()
    return out
...
def add_vep_annot_back_to_tsv(vcf_file, tsv_file, out=None):
    ...
    new = pd.DataFrame(annot_dict.values(), columns=table.columns)
    result = pd.concat([table, new]).round(4)
...
def get_insert_support_reads(bam, contig, start, ref, min_bq=0, ignore_overlaps=False, fasta_file=None):
    cols = bam.pileup(
        contig, start, start+1,
        stepper='samtools',
        truncate=True,
    ...
    for col in cols:
        for base, read in zip(
            ...
            if '+' in base:
                ...
                mismatch_allowed = round(len(ref)*0.15)
                mismatched_num = sum(x != y for x, y in zip(ref, insertion))
...
def get_complex_support_reads(bam, contig, start, ref, alt, min_bq=0, ignore_overlaps=False, fasta_file=None):
    """
    思路：提取vcf中突变起始位置的前一个碱基对应的比对信息，分析每条比对到该位置的read序列是否支持alt
    当突变是AAA->A这种del形式时，上面的思路不可行, 因为没有突变的read也将符合程序的判定
    当突变时ATC->G或ATC->GC这种complex形式时，上述思路可行。
    注意：上述思路并非严谨的寻找支持reads，还是以"ATC->G或ATC->GC"为例，容易想到支持”ATC->GC”或“AT->G"的read同样也支持"ATC->G"
    为增加严谨性：
    1. 假设alt后的2个碱基一定是和reference匹配的
    ...
    for col in cols:
        for pileup_read in col.pileups:
            ...
            if query_pos is not None:
                # 提取当前read中目标位置的序列，看其是否和alt一致
                real_seq = query_seq[query_pos+1:query_pos+1+len(alt) + 2]
                expected_seq = fasta_file.fetch(contig, start+len(ref), start+len(ref)+2)
                # print("xxx", contig, start, real_seq, alt, expected_seq)
                if real_seq == alt+expected_seq.upper():
                    support_reads.add(pileup_read.alignment.query_name)
                    # print(query_seq, query_pos, alt)
    return support_reads
...
def get_support_reads(mut, mut_type, bam, fasta):
    ...
    if mut_type == 'snp':
        ...
        supports = get_complex_support_reads(bam, mut.contig, mut.start, mut.ref, mut.alts[0], fasta_file=fasta)
        ...
        supports = get_insert_support_reads(bam, mut.contig, mut.start, mut.ref, fasta_file=fasta)
...
def distance(mut_a, mut_b):
    ...
    if mut_a.pos > mut_b.pos:
        raise Exception('第二个突变的起始坐标不能小于第一个突变的起始坐标')
    if mut_a.pos == mut_b.pos:
        # 对于同一个位点的不同突变，跳过合并
        return 10000
    if mut_b.start < mut_a.stop:
        # 可能是MSI导致的
        print(f'特殊突变：后一个突变{mut_b.pos} 在前一个突变的坐标范围内{mut_a.pos}-{mut_a.stop}')
        return 10000
...
def format_complex_variant(vcf, out=None, genome=None, bam=None, filter_by_pass=True, logger=None):
    """
    1. 要求vcf是排序好的
    ...
    if logger is None:
        log_file = os.path.join(os.path.dirname(out), 'find_complex_variants.log')
        logger = set_logger(log_file, 'find_complex_variants')

    def read_vcf(infile, keep_pass=filter_by_pass):
        # 只保留filter=PASS和alt不为空的行
        with VariantFile(infile) as temp:
            vcf_header = temp.header
            vcf_header.info.add('MergeFrom', number=1, type='String',
                                description='Merged Variant Support Number | VariantPos | VariantPos')
            for r in temp:
                if keep_pass:
    ...
    with VariantFile(vcf) as fr:
        header = fr.header
        header.info.add('MergeFrom', number=1, type='String',
                        description='Merged Variant Support Number | VariantPos | VariantPos')
        ...
        while True:
            ...
            if dis <= 2 and mut_a_is_s and mut_b_is_s:
                ...
                if len(intersection) >= 3:
                    ...
                    # 搜索支持merge突变的reads
                    mut_c_supports = get_support_reads(record, "complex", bam, fasta)
