...
class VardictFilter():
    ...
    def get_binomial_pvalue(self, alt_depth, depth, error_rate):
        # 假设错误服从二型分布，计算alt_depth全部来自错误的概率
    ...
    def filtering(self, genome, out_prefix=None, seq_error=None, tumor_index=1, center_size:tuple=None, basic_error_dict=None):
        ...
        self.vcf.header.info.add(
            'LOD', number=2, type='Float',
            description='The first value is input error rate which will be used as theoretical frequency to '
                        'calculate the second value. The second value is the upper value of estimated error rate'
        ...
        if normal is not None:
            if 'DepthBias' not in self.vcf.header.filters:
                self.vcf.header.filters.add('DepthBias', number=None, type=None, description="not enough depth bias between norm and tumor")
