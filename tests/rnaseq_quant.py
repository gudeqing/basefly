...
def pipeline():
    ...
    """
    wf = Workflow()
    wf.meta.name = 'RNAQuant'
    wf.meta.source = "you may provide a link here"
    wf.meta.desc = """
        fast gene quantification using fastp and salmon
    ...
    for sample, (r1, r2) in fastq_info.items():
        # 向流程中添加task
        fastp_task, args = wf.add_task(fastp(), name='fastp-'+sample, parent_wkdir='fastp')
        ...
        # 添加第二个task
        task, args = wf.add_task(salmon(), tag=sample, parent_wkdir='salmon')
    ...
    # 你也可以选择生成其他版本的流程
    # wf.to_argo_workflow(f'{wf.meta.name}.yaml')
