...
all_in_one_image = 'gudeqing/dnaseq:1.0'
...
def creat_ref_dict():
    ...
    cmd.args['copy_input_mode'] = Argument(prefix=f'cp -', default='L', range=['L', 'l', 's'],
                                           desc='indicate how to copy input fasta into work directory, "L": copy, "l": hard link, "s": softlink, do not use this if docker is used')
    ...
    cmd.args['_ref_dict'] = Argument(prefix='-O ', type='fix', value='ref_genome.dict',
                                     desc="Output a dict file containing only the sequence dictionary. By default it will use the base name of the input reference with the .dict extension")
    cmd.args['alt_names'] = Argument(prefix='-ALT_NAMES ', type='infile', level='optional',
                                     desc="Optional file containing the alternative names for the contigs. Tools may use this information to consider different contig notations as identical (e.g: 'chr1' and '1'). The alternative names will be put into the appropriate @AN annotation for each contig. No header. First column is the original name, the second column is an alternative name. One contig may have more than one alternative name. Default value: null.")
...
def build_bwa_index():
    ...
    cmd.args['copy_input_mode'] = Argument(prefix=f'cp -', default='s', range=['L', 'l', 's'],
                                           desc='indicate how to copy input fasta into work directory, "L": copy, "l": hard link, "s": softlink')
    cmd.args['ref_fasta'] = Argument(prefix='', type='infile', desc='reference fasta to index')
    cmd.args['_new_name'] = Argument(type='fix', value='ref_genome.fa && samtools faidx ref_genome.fa &&')
    ...
    cmd.args['_create_dict'] = Argument(type='fix', value='&& gatk CreateSequenceDictionary')
    cmd.args['_reference'] = Argument(prefix='-R ', type='fix', value='ref_genome.fa', desc='reference fasta')
    cmd.args['ref_dict'] = Argument(prefix='-O ', default='ref_genome.dict',
                                    desc="Output a dict file containing only the sequence dictionary. By default it will use the base name of the input reference with the .dict extension")
    cmd.args['alt_names'] = Argument(prefix='-ALT_NAMES ', type='infile', level='optional',
                                     desc="Optional file containing the alternative names for the contigs. Tools may use this information to consider different contig notations as identical (e.g: 'chr1' and '1'). The alternative names will be put into the appropriate @AN annotation for each contig. No header. First column is the original name, the second column is an alternative name. One contig may have more than one alternative name. Default value: null.")
    cmd.outputs['ref_genome'] = Output(value='ref_genome.fa')
...
def FastqToSam(sample):
    ...
    cmd.args['tmpdir'] = Argument(prefix='--TMP_DIR ', default='.',
                                  desc='directorie with space available to be used by this program for temporary storage of working files')
    cmd.outputs['out'] = Output(value='{out}')
    return cmd
...
def uBam2FastqBwaMem(sample):
    ...
    cmd.args['paired'] = Argument(prefix='--INTERLEAVE ', default='true', range=['true', 'false'],
                                  desc='if input is paired fastq, set it be true, else set it be false')
    cmd.args['_fix1'] = Argument(type='fix', value='--FASTQ /dev/stdout -NON_PF true')
    cmd.args['_fix2'] = Argument(type='fix', value='| /opt/bwa-mem2-2.2.1_x64-linux/bwa-mem2 mem -M -Y -p -v 3',
                                 desc='this is bwa base command')
...
def MergeBamAlignment(sample):
    ...
    cmd.args['REFERENCE_SEQUENCE'] = Argument(prefix='--REFERENCE_SEQUENCE ', type='infile',
                                              desc='reference fasta file')
    cmd.args['SORT_ORDER'] = Argument(prefix='--SORT_ORDER ', default='"unsorted"')
    cmd.args['CLIP_ADAPTERS'] = Argument(prefix='--CLIP_ADAPTERS ', default='false',
                                         desc='Whether to clip adapters where identified.')
    cmd.args['MAX_RECORDS_IN_RAM'] = Argument(prefix='--MAX_RECORDS_IN_RAM ', default=2000000,
                                              desc='When writing files that need to be sorted, this will specify the number of records stored in RAM before spilling to disk. Increasing this number reduces the number of file handles needed to sort the file, and increases the amount of RAM needed.')
    cmd.args['MAX_INSERTIONS_OR_DELETIONS'] = Argument(prefix='--MAX_INSERTIONS_OR_DELETIONS ', default=-1,
                                                       desc='The maximum number of insertions or deletions permitted for an alignment to be included.')
    cmd.args['PRIMARY_ALIGNMENT_STRATEGY'] = Argument(prefix='--PRIMARY_ALIGNMENT_STRATEGY ', default='MostDistant')
    cmd.args['UNMAPPED_READ_STRATEGY'] = Argument(prefix='--UNMAPPED_READ_STRATEGY ', default='COPY_TO_TAG',
                                                  desc='How to deal with alignment information in reads that are being unmapped (e.g. due to cross-species contamination.) Currently ignored unless UNMAP_CONTAMINANT_READS = true. Note that the DO_NOT_CHANGE strategy will actually reset the cigar and set the mapping quality on unmapped reads since otherwisethe result will be an invalid record. To force no change use the DO_NOT_CHANGE_INVALID strategy.')
    cmd.args['ALIGNER_PROPER_PAIR_FLAGS'] = Argument(prefix='--ALIGNER_PROPER_PAIR_FLAGS ', default='true',
                                                     desc="Use the aligner's idea of what a proper pair is rather than computing in this program.")
    cmd.args['UNMAP_CONTAMINANT_READS'] = Argument(prefix='--UNMAP_CONTAMINANT_READS ', default='true',
                                                   desc='Detect reads originating from foreign organisms (e.g. bacterial DNA in a non-bacterial sample),and unmap + label those reads accordingly.')
    ...
    cmd.args['PROGRAM_GROUP_COMMAND_LINE'] = Argument(prefix='--PROGRAM_GROUP_COMMAND_LINE ',
                                                      default='"bwa-mem2 mem -M -Y -p -v 3 -K 10000000 -t 4 ref.fa"')
...
def MarkDuplicates(sample):
    ...
    cmd.args['OUTPUT'] = Argument(prefix='--OUTPUT ', default=f'{sample}.unsorted.dup_marked.bam',
                                  desc='output bam file')
    cmd.args['METRICS_FILE'] = Argument(prefix='--METRICS_FILE ', default=f'{sample}.dup_metrics.txt')
    cmd.args['VALIDATION_STRINGENCY'] = Argument(prefix='--VALIDATION_STRINGENCY ', default='SILENT')
    cmd.args['OPTICAL_DUPLICATE_PIXEL_DISTANCE'] = Argument(prefix='--OPTICAL_DUPLICATE_PIXEL_DISTANCE ', default=2500,
...
def SortAndFixTags(sample):
    ...
    cmd.args['tmpdir'] = Argument(prefix='--TMP_DIR ', default='.',
                                  desc='directorie with space available to be used by this program for temporary storage of working files')
    cmd.args['_fix_tags'] = Argument(type='fix',
                                     value='| gatk --java-options -Xmx15g SetNmMdAndUqTags --CREATE_INDEX true --INPUT /dev/stdin')
...
def BaseRecalibrator(sample):
    ...
    cmd.args['OUTPUT'] = Argument(prefix='-O ', default=f'{sample}.recal_table',
                                  desc='The output recalibration table file to create')
    cmd.args['known-sites'] = Argument(prefix='--known-sites ', type='infile', multi_times=True,
                                       desc='One or more databases of known polymorphic sites used to exclude regions around known polymorphisms from analysis.')
    cmd.args['intervals'] = Argument(prefix='--intervals ', type='str', multi_times=True,
                                     desc='One or more genomic intervals over which to operate')
    cmd.outputs['out'] = Output(value='{OUTPUT}')
    return cmd
...
def ApplyBQSR(sample):
    ...
    cmd.args['INPUT'] = Argument(prefix='-I ', type='infile', desc='input bam file')
    cmd.args['OUTPUT'] = Argument(prefix='-O ', default=f'{sample}.recalibrated.bam',
                                  desc='The output recalibration table file to create')
    cmd.args['REFERENCE_SEQUENCE'] = Argument(prefix='-R ', type='infile', desc='reference fasta file')
    cmd.args['intervals'] = Argument(prefix='-L ', type='str', multi_times=True,
                                     desc='One or more genomic intervals over which to operate')
    cmd.args['bqsr'] = Argument(prefix='-bqsr ', type='infile', desc='Input recalibration table for BQSR')
    cmd.args['static-quantized-quals'] = Argument(prefix='--static-quantized-quals ', type='int', multi_times=True,
                                                  default=[10, 20, 30])
    cmd.args['add-output-sam-program-record'] = Argument(prefix='--add-output-sam-program-record', type='bool',
                                                         default=True)
    cmd.args['use-original-qualities'] = Argument(prefix='--use-original-qualities', type='bool', default=True)
    cmd.outputs['out'] = Output(value='{OUTPUT}')
    return cmd
...
def GatherBamFiles(sample):
    ...
    cmd.args['REFERENCE_SEQUENCE'] = Argument(prefix='-R ', type='infile', level='optional',
                                              desc='reference fasta file')
...
def SplitIntervals(scatter_number=10):
    ...
    cmd.args['REFERENCE_SEQUENCE'] = Argument(prefix='-R ', type='infile', desc='reference fasta file')
    cmd.args['intervals'] = Argument(prefix='-L ', level='optional', type='infile', multi_times=True,
                                     desc='One or more genomic intervals over which to operate')
    cmd.args['scatter'] = Argument(prefix='-scatter ', type='fix', value=scatter_number,
                                   desc='number of output interval files to split into')
    ...
    cmd.args['outdir'] = Argument(prefix='-O ', default='intervals_folder',
                                  desc='The directory into which to write the scattered interval sub-directories.')
...
def Mutect2(prefix):
    ...
    cmd.args['germline-resource'] = Argument(prefix='--germline-resource ', type='infile', level='optional',
                                             desc='optional database of known germline variants (and its index) (see http://gnomad.broadinstitute.org/downloads)')
    cmd.args['pon'] = Argument(prefix='-pon ', type='infile', level='optional', desc='')
    cmd.args['intervals'] = Argument(prefix='-L ', type='infile', level='optional', multi_times=True,
                                     desc='One or more genomic intervals over which to operate')
    cmd.args['string_intervals'] = Argument(prefix='-L ', level='optional', desc='interval string such as "chrM"')
    cmd.args['alleles'] = Argument(prefix='--alleles ', type='infile', level='optional',
                                   desc='The set of alleles to force-call regardless of evidence')
    ...
    cmd.args['f1r2-tar-gz'] = Argument(prefix='--f1r2-tar-gz ', type='infile', default=f'{prefix}.f1r2.tar.gz',
                                       desc='If specified, collect F1R2 counts and output files into this tar.gz file')
    cmd.args['mitochondria'] = Argument(prefix='--mitochondira', type='bool',
                                        desc='if to turn on mitochondria mode. Specifically, the mode sets --initial-tumor-lod to 0, --tumor-lod-to-emit to 0, --af-of-alleles-not-in-resource to 4e-3, and the advanced parameter --pruning-lod-thres')
    cmd.args['tmpdir'] = Argument(prefix='--tmp-dir ', default='.',
                                  desc='directorie with space available to be used by this program for temporary storage of working files')
    cmd.outputs['out'] = Output(value='{out}')
...
def bcftools_norm():
    ...
    cmd.args['multiallelics'] = Argument(prefix='-m ', level='optional',
                                         desc='Split multiallelics (-) or join biallelics (+), type: snps|indels|both|any [both]')
    cmd.args['out'] = Argument(prefix='-o ', desc='Write output to a file [standard output]')
    cmd.args['output-type'] = Argument(prefix='--output-type ', default='v',
                                       desc="'b' compressed BCF; 'u' uncompressed BCF; 'z' compressed VCF; 'v' uncompressed VCF [v]")
    cmd.args['threads'] = Argument(prefix='--threads ', default=2,
                                   desc="Use multithreading with <int> worker threads [2]")
...
def GetPileupSummaries(prefix):
    ...
    cmd.args['interval-set-rule'] = Argument(prefix='--interval-set-rule ', default='INTERSECTION',
                                             desc='Set merging approach to use for combining interval inputs')
    cmd.args['intervals'] = Argument(prefix='-L ', type='infile', multi_times=True,
                                     desc='One or more genomic intervals over which to operate')
    cmd.args['variants_for_contamination'] = Argument(prefix='-V ', type='infile',
                                                      desc='A VCF file containing variants and allele frequencies')
    cmd.args['out'] = Argument(prefix='-O ', type='infile', default=f'{prefix}.tumor-pileups.table',
...
def MergeVcfs(prefix):
    ...
    cmd.args['out'] = Argument(prefix='-O ', default=f'{prefix}.vcf.gz',
                               desc='The merged VCF or BCF file. File format is determined by file extension.')
    cmd.outputs['out'] = Output(value='{out}')
    return cmd
...
def LearnReadOrientationModel(sample):
    ...
    cmd.args['inputs'] = Argument(prefix='-I ', type='infile', multi_times=True,
                                  desc='One or more .tar.gz containing outputs of CollectF1R2Counts')
    cmd.args['out'] = Argument(prefix='-O ', default=f'{sample}.artifact-priors.tar.gz',
                               desc='tar.gz of artifact prior tables')
    cmd.outputs['out'] = Output(value='{out}')
    return cmd
...
def CalculateContamination(prefix):
    ...
    cmd.args['tumor-segmentation'] = Argument(prefix='--tumor-segmentation ', default=f'{prefix}.tumor_segmentation',
                                              desc='The output table containing segmentation of the tumor by minor allele fraction')
    cmd.outputs['out'] = Output(value='{out}')
...
def FilterMutectCalls(sample):
    ...
    cmd.args['filtering-stats'] = Argument(prefix='--filtering-stats ', default=f'{sample}.filtering.stats',
                                           desc='output filtering stat file')
    cmd.outputs['out'] = Output(value='{out}')
...
def FilterAlignmentArtifacts(sample):
    ...
    cmd.args['bwa-mem-index-image'] = Argument(prefix='--bwa-mem-index-image ', type='infile',
                                               desc='BWA-mem index image')
    cmd.args['out'] = Argument(prefix='-O ', default=f'{sample}.align_artifacts_filtered.vcf.gz',
                               desc='output vcf file')
    cmd.outputs['out'] = Output(value='{out}')
    return cmd
...
def Haplotyper(sample):
    ...
    cmd.args['intervals'] = Argument(prefix='-L ', level='optional', type='infile', multi_times=True,
                                     desc="interval file, support bed file or picard interval or vcf format")
    cmd.args['bam'] = Argument(prefix='-I ', type='infile', desc='reccaled tumor and normal bam list')
    cmd.args['REFERENCE_SEQUENCE'] = Argument(prefix='-R ', type='infile', desc='reference fasta file')
    cmd.args['emit_mode'] = Argument(prefix='-ERC ', default='GVCF', range=['NONE', 'GVCF', 'BP_RESOLUTION'],
                                     desc='The reference confidence mode makes it possible to emit a per-bp or summarized confidence estimate for a site being strictly homozygous-reference.')
    cmd.args['ploidy'] = Argument(prefix='-ploidy ', type='int', default=2,
                                  desc='determines the ploidy number of the sample being processed. The default value is 2.')
    cmd.args['annotation-group'] = Argument(prefix='-G ', multi_times=True,
                                            default=['StandardAnnotation', 'StandardHCAnnotation',
                                                     'AS_StandardAnnotation'])
    cmd.args['gvcf-gq-bands'] = Argument(prefix='-GQB ', multi_times=True, default=[10, 20, 30, 40, 50, 60, 70, 80, 90],
                                         desc='Exclusive upper bounds for reference confidence GQ bands (must be in [1, 100] and specified in increasing order)')
    cmd.args['tmpdir'] = Argument(prefix='--tmp-dir ', default='.',
                                  desc='directorie with space available to be used by this program for temporary storage of working files')
...
def GenomicsDBImport():
    ...
    cmd.args['genomicsdb-workspace-path'] = Argument(prefix='--genomicsdb-workspace-path ', default='genomicsdb',
                                                     desc='Workspace for GenomicsDB. Can be a POSIX file system absolute or relative path or a HDFS/GCS URL. Use this argument when creating a new GenomicsDB workspace. Either this or genomicsdb-update-workspace-path must be specified. Must be an empty or non-existent directory.')
...
def GenotypeGVCFs(prefix):
    ...
    * Output:
    ...
    cmd.args['only-output-calls-starting-in-intervals'] = Argument(prefix='--only-output-calls-starting-in-intervals',
...
def VariantFiltration(prefix):
    ...
    cmd.args['filters'] = Argument(prefix='',
                                   default=['--filter-name ExcessHet --filter-expression "ExcessHet > 54.69"', ],
                                   multi_times=True, desc='filtering expressions, such as "AB < 0.2"')
...
def IndelsVariantRecalibrator(prefix):
    ...
    cmd.args['out'] = Argument(prefix='-O ', default=f'{prefix}.indel.recal',
                               desc='The output recal file used by ApplyVQSR')
    cmd.args['out-tranches-file'] = Argument(prefix='--tranches-file ', default=f'{prefix}.indel.tranches',
                                             desc='The output tranches file used by ApplyVQSR')
    cmd.args['tranche'] = Argument(prefix='-tranche ', multi_times=True,
                                   default=["100.0", "99.95", "99.9", "99.5", "99.0", "97.0", "96.0", "95.0", "94.0",
                                            "93.5", "93.0", "92.0", "91.0", "90.0"],
                                   desc='recalibration tranche values')
    cmd.args['use-annotation'] = Argument(prefix='-an ', multi_times=True,
                                          default=["AS_FS", "AS_ReadPosRankSum", "AS_MQRankSum", "AS_QD", "AS_SOR"],
                                          desc='The names of the annotations which should used for calculations')
    cmd.args['trust-all-polymorphic'] = Argument(prefix='--trust-all-polymorphic', type='bool', default=True)
    cmd.args['use-allele-specific-annotations'] = Argument(prefix='--use-allele-specific-annotations', type='bool',
                                                           default=True)
    cmd.args['mode'] = Argument(prefix='-mode ', type='fix', default='INDEL', desc='Recalibration mode to employ')
    cmd.args['max-gaussians'] = Argument(prefix='--max-gaussians ', default=4,
                                         desc='Max number of Gaussians for the positive model')
    cmd.args['mills'] = Argument(prefix='-resource:mills,known=false,training=true,truth=true,prior=12 ', type='infile',
                                 desc='mills resource vcf')
    cmd.args['axiomPloly'] = Argument(prefix='-resource:axiomPoly,known=false,training=true,truth=false,prior=10 ',
                                      type='infile', desc='axiomPoly resource vcf')
    cmd.args['dbsnp'] = Argument(prefix='-resource:dbsnp,known=true,training=false,truth=false,prior=2 ', type='infile',
                                 desc='dbsnp resource vcf')
    cmd.outputs['out'] = Output(value='{out}')
...
def SNPsVariantRecalibrator(prefix):
    ...
    cmd.runtime.tool = 'gatk --java-options -Xmx15g VariantRecalibrator'
    cmd.args['vcf'] = Argument(prefix='-V ', type='infile', desc='site only variant filtered input vcf')
    cmd.args['out'] = Argument(prefix='-O ', default=f'{prefix}.snp.recal',
                               desc='The output recal file used by ApplyVQSR')
    cmd.args['out-tranches-file'] = Argument(prefix='--tranches-file ', default=f'{prefix}.snp.tranches',
                                             desc='The output tranches file used by ApplyVQSR')
    cmd.args['tranche'] = Argument(prefix='-tranche ', multi_times=True,
                                   default=["100.0", "99.95", "99.9", "99.8", "99.7", "99.6", "99.5", "99.4", "99.3",
                                            "99.0", "98.0", "97.0", "90.0"], desc='recalibration tranche values')
    cmd.args['use-annotation'] = Argument(prefix='-an ', multi_times=True,
    ...
    cmd.args['max-gaussians'] = Argument(prefix='--max-gaussians ', default=6,
                                         desc='Max number of Gaussians for the positive model')
    cmd.args['output-model'] = Argument(prefix='--output-model ', level='optional',
                                        desc='If specified, the variant recalibrator will output the VQSR model to this file path.')
    cmd.args['hapmap'] = Argument(prefix='-resource:hapmap,known=false,training=true,truth=true,prior=15 ',
                                  type='infile', desc='hapmap resource vcf')
    cmd.args['omni'] = Argument(prefix='-resource:omni,known=false,training=true,truth=true,prior=12 ', type='infile',
                                desc='omni resource vcf')
    cmd.args['1000G'] = Argument(prefix='-resource:1000G,known=false,training=true,truth=false,prior=10 ',
                                 type='infile', desc='one thousand genomes resource vcf')
    cmd.args['dbsnp'] = Argument(prefix='-resource:dbsnp,known=true,training=false,truth=false,prior=7 ', type='infile',
                                 desc='dbsnp resource vcf')
    cmd.outputs['out'] = Output(value='{out}')
    cmd.outputs['tranches'] = Output(value='{out-tranches-file}')
...
def ApplyVQSR(prefix):
    ...
    cmd.args['recal-file'] = Argument(prefix='--recal-file ', type='infile',
                                      desc='The input recal file used by ApplyVQSR')
    cmd.args['use-allele-specific-annotations'] = Argument(prefix='--use-allele-specific-annotations ', type='bool',
                                                           default=True)
    cmd.args['tranches-file'] = Argument(prefix='--tranches-file ', type='infile',
                                         desc='The input tranches file used by ApplyVQSR')
    cmd.args['truth-sensitivity-filter-level'] = Argument(prefix='--truth-sensitivity-filter-level ', default=99.0,
                                                          desc='The truth sensitivity level at which to start filtering')
    cmd.args['create-output-variant-index'] = Argument(prefix='--create-output-variant-index ', default='true',
                                                       range=['true', 'false'])
    cmd.args['mode'] = Argument(prefix='-mode ', default='SNP', range=['SNP', 'INDEL'],
                                desc="Recalibration mode to employ: 1.) SNP for recalibrating only SNPs (emitting indels untouched in the output VCF); 2.) INDEL for indels; and 3.) BOTH for recalibrating both SNPs and indels simultaneously.")
    cmd.args['tmpdir'] = Argument(prefix='--tmp-dir ', default='.',
                                  desc='directorie with space available to be used by this program for temporary storage of working files')
    cmd.outputs['out'] = Output(value='{out}')
    return cmd
...
def pipeline():
    wf = Workflow()
    wf.meta.version = "0.1"
    wf.meta.name = 'GATK-WES-Workflow'
    wf.meta.source = """
    流程主要参考来源有:
    ...
    * 运行命令: python /enigma/datasets/*/scripts/wgs/wes.dnaseq.py -fastq_info /enigma/datasets/*/your_testdata_dir -r1_name '(.*?)_S*.R2.fastq.gz' -r2_name '(.*?)_S*.R2.fastq.gz' -ref_dir /enigma/datasets/*/hg38/ -intervals /enigma/datasets/*/hg38/wes.agilent_region.B37_V6.toHg38.bed  -outdir /enigma/local_storage/Result
    * 分析结果：主要分析结果将汇总于输出目录如Result/Report

    主要输入说明（以hg38为例）:
    1. 大部分的输入文件可以从aws提供的公开数据集获得: aws s3 ls --no-sign-request s3://broad-references/hg38/v0/
        期望得到如下文件:
    ...
    2. 索引文件：执行如下命令可以获得，/opt/bwa-mem2-2.2.1_x64-linux/bwa-mem2 index your_fasta_file
    3. fastq文件, 可以提供一个fastq所在目录,然后通过参数'-r1_name'和'-r2_name'提供fastq的文件名的正则表达式, 我们依据正则表达式提取样本路径和样本名称
    4. WES的捕获区域文件，可以提供bed格式或interval格式文件
    """
    # 定义流程输入参数
    wf.init_argparser()
    ...
    wf.add_argument('-intervals', required=True, help="Interval file, support bed file or picard interval file. This is required for WES or large panel sequencing")
    wf.add_argument('-ref_dir', default='/enigma/datasets/*/hg38/',
                    help='The directory containing all reference files for gatk input. The following input files need to be in this file')
    wf.add_argument('--ref', default='Homo_sapiens_assembly38.fasta',
                    help='reference fasta file. It should be in "ref_dir"')
    wf.add_argument('--dbsnp', default='dbsnp_146.hg38.vcf.gz', help='dbsnp vcf file')
    wf.add_argument('--axiomPoly', default='Axiom_Exome_Plus.genotypes.all_populations.poly.hg38.vcf.gz',
                    help='high confidence known indel vcf file, such as Axiom_Exome_Plus.genotypes.all_populations.poly.vcf.gz. 1,249 individuals are included, drawn from the International HapMap Project and 1000 Genomes Project sample collection. https://www.thermofisher.cn/cn/zh/home/life-science/microarray-analysis/microarray-data-analysis/microarray-analysis-sample-data/axiom-exome-sample-data-set.html')
    wf.add_argument('--mills', default="Mills_and_1000G_gold_standard.indels.hg38.vcf.gz",
                    help='high confidence known indel vcf file. such as Mills_and_1000G_gold_standard.indels.hg38.vcf')
    wf.add_argument('--hapmap', default="hapmap_3.3.hg38.vcf.gz",
                    help='high confidence known snp vcf file. 来自国际人类单倍体型图计划, 这个数据集包含了大量家系数据，并且有非常严格的质控和严密的实验验证，因此它的准确性是目前公认最高的')
    wf.add_argument('--omni', default="1000G_omni2.5.hg38.vcf.gz",
                    help='high confidence known snp vcf file. 这个数据源自Illumina的Omni基因型芯片，大概2.5百万个位点，它的验证结果常常作为基因型的金标准')
    wf.add_argument('--G1000', default="1000G_phase1.snps.high_confidence.hg38.vcf.gz",
                    help='high confidence known snp vcf file. source from 1000 genomes project')
    wf.add_argument('--bwaMemIndexImage', required=False,
                    help='bwa-mem-index-mage for artifact alignment filtering. you may created it with tool BwaMemIndexImageCreator with only fasta as input')
    ...
    top_vars = dict(
        ...
        bwaMemIndexImage=TopVar(
            value=os.path.join(wf.args.ref_dir, wf.args.bwaMemIndexImage) if wf.args.bwaMemIndexImage else None,
            type='infile'),
    ...
    print(
        'Caution: This mode is suitable for WES and WGS or large panel. For small panel, error may happen in recalibration stage!')
    ...
    if not os.path.exists(dict_file) or not os.path.exists(fai_file):
        ...
        creat_dict_task.run_now(wkdir=os.path.abspath(tmp_wkdir), docker=wf.args.docker)
    ...
    if input_interval and input_interval.endswith('.bed'):
        ...
        bed2interval_task.run_now(wkdir=os.path.abspath(os.path.join(wf.args.outdir, "PreparedInputs")), docker=wf.args.docker)
    ...
    for sample, reads in fastq_info.items():
        ...
        for ind, (r1, r2) in enumerate(zip(r1s, r2s)):
            ...
            # merge
            merge_bam_task, args = wf.add_task(MergeBamAlignment(sample_tag), tag=sample_tag,
                                               depends=[fastq2sam_task, bwa_task])
        ...
        for ind, each in enumerate(seq_groups):
            bsqr_task, args = wf.add_task(BaseRecalibrator(f'{sample}-g{ind}'), tag=f'{sample}-g{ind}',
                                          depends=[sort_task], parent_wkdir='BaseRecalibrator')
        ...
        for ind, each in enumerate(seq_groups2):
            apply_task, args = wf.add_task(ApplyBQSR(f'{sample}-g{ind}'), tag=f'{sample}-g{ind}',
                                           parent_wkdir='ApplyBQSR')
    ...
    for tumor_sample, normal_sample in pair_list:
        ...
        if normal_sample not in bam_dict and normal_sample.lower() != 'none':
            print(
                f'Warning: skip normal sample {normal_sample} since it is not in target list: {list(bam_dict.keys())}')
            continue

        # germline variant calling
        if normal_sample.lower() != 'none' and (normal_sample not in already_exist) and (
                wf.args.call_mode in ['mix', 'germline']):
    ...
    if germline_merge_vcf_tasks:
        ...
        split_task.run_now(wkdir=os.path.abspath(wf.args.outdir), wf_tasks=wf.tasks, docker=wf.args.docker)
    ...
    for ind, interval_file in enumerate(interval_files):
        ...
        hard_filter_task, args = wf.add_task(VariantFiltration(f'scatter{ind}'), tag=f'{ind}',
                                             parent_wkdir='VariantHardFiltration')
        args['vcf'].value = genotype_task.outputs['out']

        make_site_only_task, args = wf.add_task(MakeSitesOnlyVcf(f'scatter{ind}'), tag=f'{ind}',
                                                parent_wkdir='MakeSitesOnlyVcf')
    ...
    if scattered_tasks:
        ...
        for idx, task in enumerate(scattered_tasks):
            apply_indel_task, args = wf.add_task(ApplyVQSR(f'joint.{idx}.indel'), tag=f'Indel{idx}',
                                                 parent_wkdir='ApplyIndelRecalibration')
            ...
            apply_snp_task, args = wf.add_task(ApplyVQSR(f'joint.{idx}.snp'), tag=f'SNP{idx}',
                                               parent_wkdir='ApplySnpRecalibration')
