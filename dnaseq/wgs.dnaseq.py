...
def pipeline():
    wf = Workflow()
    wf.meta.version = "0.1"
    wf.meta.name = 'GATK-WGS-Workflow'
    wf.meta.source = """
    流程主要参考来源有:
    ...
    * 运行命令: python /enigma/datasets/*/scripts/wgs/wgs.dnaseq.py -fastq_info /enigma/datasets/*/your_testdata_dir -r1_name '(.*?)_S*.R2.fastq.gz' -r2_name '(.*?)_S*.R2.fastq.gz' -ref_dir /enigma/datasets/*/hg38/ -outdir /enigma/local_storage/Result
