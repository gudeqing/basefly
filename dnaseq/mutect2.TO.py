...
def pipeline():
    wf = Workflow()
    wf.meta.version = "0.1"
    wf.meta.name = 'Mutect2-Tumor-Only-Workflow'
    wf.meta.function = '本流程适用于对非配对的肿瘤样本进行体细胞突变检测分析'
    wf.meta.source = """
    ## https://github.com/gatk-workflows/gatk4-data-processing/tree/2.1.1
    ## https://github.com/gatk-workflows/gatk4-germline-snps-indels
    ## https://github.com/broadinstitute/warp/blob/develop/pipelines/broad/dna_seq/germline/joint_genotyping/JointGenotyping.wdl
    ...
    * 使用fastp进行测序接头自动去除
    * 使用BWA进行比对分析
    * 使用GATK检测small SNP/Indel, 仅支持tumor-only模式
    * 可选项: 基于VEP进行突变注释

    使用示例：
    * 确认已经准备好输入文件或加载所需数据集（如流程文件数据集，样本数据集，参考文件数据集）
    * 运行命令: python /enigma/datasets/*/scripts/mutect2_TN/mutect2.TN.py -fastq_info /enigma/datasets/*/your_testdata_dir -r1_name "(.*)_1.fq.gz" -r2_name "(.*)_2.fq.gz" -ref_dir /enigma/datasets/*/hg38/ -outdir /enigma/local_storage/Result -intervals /home/hxbio04/dbs/gatk_ref/hg38/wes.agilent_region.B37_V6.toHg38.bed --plot --docker --run -exclude -exclude P68_normal
    * 分析结果：主要分析结果将汇总于输出目录如Result/Report
    ...
    1. 下载aws提供的公开数据集: aws s3 ls --no-sign-request s3://broad-references/hg38/v0/
        ...
        Homo_sapiens_assembly38.known_indels.vcf.gz, 作为参数"--known_indels"的输入
        ...
        wes.agilent_region.B37_V6.toHg38.bed, 如果是WES测序,可以作为-intervals参数的输入,不同的外显子组捕获区域可能不一样，请根据实际情况输入
    ...
    4. 输入参数interval提供捕获区域文件，可以提供bed文件格式或interval文件格式
    """
    # 定义流程输入参数
    wf.init_argparser()
    ...
    wf.add_argument('--known_indels', default='Homo_sapiens_assembly38.known_indels.vcf.gz', help='known indel vcf file')
    ...
    top_vars = dict(
        ...
        known_indels=TopVar(value=os.path.join(wf.args.ref_dir, wf.args.known_indels), type='infile'),
    ...
    if input_interval and input_interval.endswith('.bed'):
        ...
        bed2interval_task.run_now(wkdir=os.path.abspath(os.path.join(wf.args.outdir, "PreparedInputs")),
                                  docker=wf.args.docker)
    ...
    for sample, reads in fastq_info.items():
        ...
        for ind, each in enumerate(seq_groups):
            ...
            args['REFERENCE_SEQUENCE'].value = wf.topvars['ref']
            args['known-sites'].value = [top_vars['dbsnp'], top_vars['mills'], top_vars['known_indels']]
